...
func run(args []string, stdout, stderr io.Writer) int {
	...
	fs.Usage = func() {
		_, _ = fmt.Fprintf(stderr, "usage: route-builder [options] routes.yaml\n")
		_, _ = fmt.Fprintf(stderr, "       route-builder [options] file.vcl [file.vcl ...]\n\nOptions:\n")
		fs.PrintDefaults()
	}
	...
	if *showVersion {
		_, _ = fmt.Fprintln(stdout, "route-builder "+Version)
		return 0
	}
	...
	case len(files) == 1 && rb.IsYAMLFile(files[0]):
		if *yamlfileOut != "" {
			_, _ = fmt.Fprintln(stderr, "error: -yamlfile is incompatible with YAML input")
			return 1
		}
		var err error
	...
	default:
		_, _ = fmt.Fprintln(stderr, "error: arguments must be a single .yaml/.yml file or one or more .vcl files")
		return 1
	}
	...
	if *testRouteURL != "" {
		...
		if err != nil {
			_, _ = fmt.Fprintf(stderr, "invalid URL %q: %v\n", *testRouteURL, err)
			return 1
		}
		host := u.Hostname()
		if host == "" {
			_, _ = fmt.Fprintf(stderr, "no hostname in URL %q\n", *testRouteURL)
			return 1
		}
		cfg := rb.FindRoute(host, configs)
		if cfg == nil {
			_, _ = fmt.Fprintf(stdout, "host %q: no route matched\n", host)
			return 1
		}
		_, _ = fmt.Fprintf(stdout, "host %q -> %s (%s)\n", host, cfg.Name, cfg.VclPath)
		return 0
	}

	if (*vclfileOut == "-" || *vclfileOut == "none") && *cmdfileOut != "none" {
		_, _ = fmt.Fprintln(stderr, "error: -vclfile - or -vclfile none requires -cmdfile none")
		return 1
	}
	...
	fs.Visit(func(f *flag.Flag) {
		if f.Name == "timeout" && !*doReload {
			_, _ = fmt.Fprintln(stderr, "warning: -timeout has no effect without -reload")
		}
	})
	...
	if *doReload {
		...
		if err != nil {
			_, _ = fmt.Fprintf(stderr, "connect to varnish: %v\n", err)
			return 1
		}
		...
		_, _ = fmt.Fprintf(stdout, "reloaded %d route(s)\n", len(configs))
	}
